...
mainApp.config(["$urlRouterProvider", "$stateProvider", function ($urlRouterProvider, $stateProvider) {
    ...
    $rootScope.$on("$stateChangeStart", function (e, to) {
        to.data && to.data.requiresLogin && (store.get("jwt") && !jwtHelper.isTokenExpired(store.get("jwt")) || (store.get("jwt") && store.remove("jwt"), e.preventDefault(), $state.go("login")))
    })
}]), mainApp.config(["$translateProvider", function ($translateProvider) {
    $translateProvider.useSanitizeValueStrategy("escape"), $translateProvider.useStaticFilesLoader({prefix: "resources/local/lang-", suffix: ".json"}), $translateProvider.preferredLanguage("en")
    ...
    $scope.back = function () {
        $scope.show_login && $state.go("workshops"), $scope.show_logout && $state.go("dashboard"), $scope.show_logout && "dashboard" == $state.current.name && $state.go("workshops"), "login" == $state.current.name && $state.go("workshops")
    };
    var jwt = store.get("jwt");
    null != jwt && jwtHelper.isTokenExpired(jwt) && store.remove("jwt"), $http.get("resources/local/config.json").then(function (response) {
        $scope.langs = response.data.lang, $translate.use(response.data["default"]);
        for (var i = 0; i < $scope.langs.length; i++)response.data["default"] === $scope.langs[i].code && ($scope.selectedLang = $scope.langs[i])
    }, function (response) {
    }), $scope.show_login = !0, $scope.show_logout = !1, $scope.$on("$stateChangeSuccess", function (event, toState, toParams, fromState, fromParams) {
        var jwt = store.get("jwt");
        "login" == toState.name ? ($scope.show_login = !1, $scope.previousState = fromState, null == jwt || jwtHelper.isTokenExpired(jwt) || (event.preventDefault(), $state.go(fromState))) : null != jwt ? ($scope.show_login = jwtHelper.isTokenExpired(jwt), $scope.show_logout = !jwtHelper.isTokenExpired(jwt)) : ($scope.show_login = !0, $scope.show_logout = !1), null != jwt && jwtHelper.isTokenExpired(jwt) && store.remove(jwt)
    }), $scope.logout = function () {
        var jwt = store.get("jwt");
        null != jwt ? (store.remove("jwt"), $scope.show_login = !0, $scope.show_logout = !1, $state.current.data.requiresLogin && $state.go("workshops")) : ($scope.show_login = !0, $scope.show_logout = !1)
    ...
    scope.title = "to be changed", scope.content = "to be changed", scope.answer = function (res) {
        deferred.resolve(res), confirm.hide()
    };
    var confirm = $modal({templateUrl: "resources/views/confirm.tpl.html", scope: scope, show: !1, dismissable: !1}), parentShow = confirm.show;
    return confirm.show = function () {
        return deferred = $q.defer(), parentShow(), deferred.promise
    }, confirm
});